function Get-Audio {
	...
	if($mciSendStringAddr -eq $null){Throw 'Failed to aquire address to mciSendStringA'}
	...
	if($mciGetErrorStringAddr -eq $null){Throw 'Failed to aquire address to mciGetErrorString'}
	...
	if($DeviceCount -gt 0){$errmsg = New-Object Text.StringBuilder 150;$rtnVal = $mciSendString.Invoke("open new Type waveaudio Alias $alias",'',0,0)
